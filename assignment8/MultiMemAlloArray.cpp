...
*/
    ...
    void pop() {
        // convert this function to de-allocate memory that was allocated using
        // malloc or calloc. quite simple actually just use the free function !
        
        int **buffer = (int **) calloc(rows, sizeof(int*));
        for ( int i {}; i < rows; ++i) {
            buffer[i] = (int*) calloc(columns, sizeof(int));
        }
        for ( int i {}; i < rows; ++i) {
	...
	// then set those values to those copied over from the buffer variable !
        arrLocation = (int**) calloc(rows, sizeof(int*));
        for (int i{0}; i < rows; ++i) {
            arrLocation[i] = (int*) calloc(columns, sizeof(int));
        }
    ...
    MultiArray() {
        ...
        // at this point in time the array is filled with random values found in memory
        std::cout << "This array has randomly been set to [" << rows - 1
	    << "][" << columns -1 << "]" << std::endl;
    }
    
    ~MultiArray() {
        // we must free up each individual allocated block of memory !
        std::cout << "MultiArray destructor called !\n";
        for (int i{0}; i < rows; i++) {
            free(arrLocation[i]);
        }
        free(arrLocation);
...
int main() {
    srand((int) time(0));
    {
        ...
        */
        ...
        anArr.printSize();
    // MultiArrary destructor will be called once it hit the line that contains the end brace
    }
    /*
    ...
    */
    ...
    //now that we're done working with the object lets de-allocate that memory !
    // since each MultiArray object is a 2D array of pointers, we must first free
    // every entry within the array first !
    // lets also create an MultiArray method to handle the free operations
    return 0;
}
